...
def parse_args():
    parser = argparse.ArgumentParser(description="A simple training script for consistency models.")
    parser.add_argument("--dataset-name",type=str,default="cifar10")
    parser.add_argument("--dataset-config-name",type=str,default=None,help="The config of the Dataset, leave as None if there's only one config.")
    parser.add_argument("--resolution",type=int,default=32,help="The resolution for input images.")
    parser.add_argument("--train-batch-size",type=int,default=16,help="Batch size (per device) for the training dataloader.")
    parser.add_argument("--dataloader-num-workers",type=int,default=8)
    parser.add_argument("--max-epochs", type=int, default=200)
    parser.add_argument("--learning-rate",type=float,default=1e-4,help="Initial learning rate (after the potential warmup period) to use.")
    parser.add_argument("--data-std",type=int,default=0.5,help="Standard deviation of the dataset")
    parser.add_argument("--time-min",type=float,default=0.002)
    parser.add_argument("--time-max",type=float,default=80)
    parser.add_argument("--bins-min",type=float,default=10)
    parser.add_argument("--bins-max",type=float,default=1280)
    parser.add_argument("--bins-rho",type=float,default=7)
    parser.add_argument("--P_mean",type=float,default=-1.1)
    parser.add_argument("--P_std",type=float,default=2)
    parser.add_argument("--log-every-n-steps",type=int,default=100)
    parser.add_argument("--sample-path",type=str,default="samples/")
...
def main(args):
    ...
    class Dataset(torch.utils.data.Dataset):
        def __init__(self, dataset_name: str, dataset_config_name=None):
            self.dataset = load_dataset(
                "parquet",
                data_files={'train':'/root/Test/CM/junhsss_consistency-models-main/examples/unconditional_image_generation/cifar10/plain_text/train-00000-of-00001.parquet',
                            'test':'/root/Test/CM/junhsss_consistency-models-main/examples/unconditional_image_generation/cifar10/plain_text/test-00000-of-00001.parquet'},
                split="train",
            )
            # self.dataset = load_dataset(
            #     dataset_name,
    ...
    consistency = ImprovedConsistency(**configs)
    trainer = Trainer(
        accelerator="auto",
        # logger=WandbLogger(project=args.wandb_project, log_model=True),
        callbacks=[
            ModelCheckpoint(
                dirpath="/root/Test/CM/diffusers_cm/scripts/ckpt",
                save_top_k=3,
                monitor="loss",
            )
        ],
...
python train_improve.py --dataset-name="/root/Test/CM/junhsss_consistency-models-main/examples/unconditional_image_generation/cifar10" --resolution=32  --train-batch-size=32 --max-epochs=100  --learning-rate=1e-4
